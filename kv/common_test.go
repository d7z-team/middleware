...
func testKVConsistency(t *testing.T, kvClient KV) {
	...
	defer func() {
		// 删除所有测试键
		keys, err := kvClient.List(ctx, uniquePrefix)
		assert.NoError(t, err)
		for key := range keys {
			_, _ = kvClient.Delete(ctx, key)
		}
		_ = kvClient.Close()
	}()
	...
	t.Run("TTL", func(t *testing.T) {
		...
		_ = kvClient.Put(ctx, ttlKey, "ttl_val", 1*time.Second)
		// 立即获取（未过期）
		...
		_, err = kvClient.Get(ctx, ttlKey)
		assert.ErrorIs(t, err, ErrKeyNotFound)

		// 永久键（TTLKeep = -1）
		keepKey := uniquePrefix + "keep_key"
		_ = kvClient.Put(ctx, keepKey, "keep_val", TTLKeep)
		// 等待 0.5 秒后获取
...
func TestMemoryKV(t *testing.T) {
	...
	t.Run("storage-scheme", func(t *testing.T) {
		...
		storageKV, err := NewMemory(tempDir)
...
func TestRedisKV(t *testing.T) {
	...
	require.NoError(t, err, "连接 Redis 失败，请检查服务是否运行")
